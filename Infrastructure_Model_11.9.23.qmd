...
```
## Updated 11-9-23; Change made: removed *z_uc_percent* 
## DV Histogram (Broadband)

```{r, tidy = TRUE, tidy.opts = list(width.cutoff=60)}
ggplot(data = Z_and_Final_Variables_10.28.22, mapping = aes(x = z_ACS_PCT_BROADBAND)) + geom_histogram() + labs(y = NULL)
```

```{r}
summary(Z_and_Final_Variables_10.28.22$ACS_PCT_BROADBAND)
```
...
# Infrastructure Model Groupings

## IV: Demographics (9 variables), DV: Broadband
...
## IV: Food & Housing (14 variables), DV: Broadband
...
## IV: Education (3 variables), DV: Broadband
...
## IV: Technology (2 variables), DV: Broadband
...
## IV: HWCC (7 variables), DV: Broadband
...
## IV: Health A (7 variables), DV: Broadband

```{r, tidy = TRUE, tidy.opts = list(width.cutoff=60)}
z_bb_just_healthA <- lm(formula = z_ACS_PCT_BROADBAND ~ z_phys_per_cap + z_mhp_per_cap + z_dents_per_cap + z_uninsur_perc + z_opcp_rate + z_TeenBirths, data = Z_and_Final_Variables_10.28.22)
...
## IV: Health B (5 variables), DV: Broadband
...
## IV: Business (4 variables), DV: Broadband
...
## IV: Government (5 variables), DV: Broadband
